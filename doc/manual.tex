...
\usepackage[english, english]{babel} % Neue Rechtschreibung f"ur Kapitel, trennung
\usepackage[utf8]{inputenc} %eingabe mit noch mehr umlauten
\usepackage[T1]{fontenc}       % 
\usepackage[a4paper,left=1.5cm,right=1.5cm,top=2.3cm,bottom=2.3cm]{geometry} %Seitenr"ander

\usepackage{color}			% falls Farbe verwendet werden soll...
\usepackage{amsmath}		% verbesserter Mathe- Formelsatz
\usepackage{graphicx}		% Graphik-unterst"utzung
\usepackage{setspace}		% zeilenabstand
\usepackage{amsfonts}		% schrift mit doppelstrich
%\usepackage{yfonts}			% frakturschreibweise

\usepackage{ifthen} % unterscheiden	
...
%\numberwithin{equation}{section}%Setzt den equation-Zaehler nach jedem Kapitel zurueck
%\numberwithin{section}{part}%Setzt den equation-Zaehler nach jedem Kapitel zurueck

\renewcommand{\theequation}{\arabic{section}.\arabic{equation}}%Definiert den Stil der Gleichungsangaben
\newcommand{\eref}[1]{Gl. (\ref{#1})} %Referenz zu Gleichung
\newcommand{\mref}[1]{(\ref{#1})} 	%Referenz zu Gleichung ohne Gl. 
\newcommand{\pref}[1]{Abbildung \ref{#1}}
\numberwithin{footnote}{page} %Fussnotenz"ahler auf jeder Seite zur"uckstellen
\renewcommand{\thefootnote}{\arabic{footnote}} % Fussnoten-Stil:)

\newcommand{\notice}[1]{\textcolor{red}{\em (#1)}} % for red notices while working
\newcommand{\source}[2]{\footnote{Quelle \cite{#1}{, #2}}}

\newboolean{onKW} %Deklaration
\setboolean{onKW}{false} %Zuweisung
...
\date{ \copyright\today}				%Erstellungsdatum
...
Commands starting with \$LUA\$ will be parsed and executed a Lua-code in the LuaVM.
...
Some functions are predefined in the mod and therefore these builtin commands can be easy renamed and unlock for different levels.
